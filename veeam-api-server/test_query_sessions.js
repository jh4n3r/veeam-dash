...
const scriptPath = path.join(__dirname, 'test_query_sessions.ps1');
...
$remoteScript = {
    ...
    $cmdlets = @("Get-VBRBackupSession", "Get-VBRTapeSession", "Get-VBRComputerBackupJobSession", "Get-VBRReplicaSession", "Get-VBRCopySession", "Get-VBRNASBackupSession", "Get-VBRSureBackupSession")
    
    foreach ($cmd in $cmdlets) {
        if (Get-Command $cmd -ErrorAction SilentlyContinue) {
            try {
                $sessions = Invoke-Expression "$cmd" | Where-Object { $_.CreationTime -ge $since }
                $summary[$cmd] = @{
                    Count = @($sessions).Count
                    SampleNames = @($sessions | Select-Object -First 3 -ExpandProperty Name)
                }
            } catch {
                $summary[$cmd] = @{ Error = $_.Exception.Message }
            }
        } else {
            $summary[$cmd] = @{ Exists = $false }
        }
    }
